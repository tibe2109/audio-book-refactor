...
replacements = {
    "Rodolpho Guglielmi": "Rô đôn phô Gúc li eo mi",
    "Guglielmi": "Gúc li eo mi",
    "Manhattan": "Man hát tan",
    "Rodolpho di Valentina": "Rô đôn phô di Va len ti na",
    "Los Angeles": "Lốt An giơ lét",
    "Hollywood": "Hô li út",
    "Rudolph Valentino": "Ru đốp Va len ti nô",
    ...
    "New York": "Niu Ước",
    "Beaucaire": "Bô ke",
    "Elvis Presley": "Eo vít Pơ rét ly",
    "Pastor Henrik Gillot": "Pát tơ Hen rích Ghi lót",
    "Gillot": "Ghi lót",
    "St. Petersburg": "Xanh Pê tét bua",
    "Lou von Salomé": "Lu von Sa lô mê",
    "Lou Andreas-Salomé": "Lu An đrê át Sa lô mê",
    "Salomé": "Sa lô mê",
    "Salome": "Sa lô mê",
    "Friedrich Nietzsche": "Phờ ri đờ rích Nít chơ",
    "Nietzsche": "Nít chơ",
    "Nietzche": "Nít chơ",
    "Genoa": "Giê noa",
    "Paul Rée": "Pôn Rê",
    ...
    "Orta": "Oóc ta",
    "Sacro": "Xác rồ",
    "Thus Spake Zarathustra": "Đất Xpếch Da ra thút tra",
    "Berlin": "Béc lin",
    "GerHauptmann": "Gờ Háp man",
    "Franz Wedekind": "Phờ ran Vê đê kin",
    "Rainer Maria Rilke": "Rai nơ Ma ri a Riu cơ",
    "Rilke": "Riu cơ",
    "Poul Bjerre": "Pôn Bờ de rơ",
    "Colette": "Cô lét",
    "La Mã": "La Mã",
    "Nhật": "Nhật",
    "Alcibiades": "An xi bi a đét",
    "Korechika": "Cô rê chi ca",
    "Beau Brummel": "Bô Bờ rum meo",
    "Brummel": "Bờ rum meo",
    "London": "Luân Đôn",
    "Théophile Gautier": "Tê ô phin Gô ti ê",
    "Oscar Wilde": "Óc ca Oai",
    "Andy Warhol": "An đi Go hon",
    "Benjamin Disraeli": "Ben gia min Đít ra e li",
    "Benjamine Disraeli": "Ben gia min Đít ra e li",
    "Disraeli": "Đít ra e li",
    "George Sand": "Gioóc giơ San",
    "Paris": "Pa ri",
    "d’Orsay": "Đoóc xây",
    "Louis XIV": "Lu y mười bốn",
    "La Bruyère": "La Bờ ru de",
    "Barbey d’Aurevilly": "Bác bây Đo rê vi ly",
    "Wilde": "Oai",
    "Rothschild": "Rót chai",
    "franc": "phờ răng",
    "Petronius": "Pê trô ni út",
    "Satyricon": "Sa ti ri con",
    "Nero": "Nê rô",
    "Seneca": "Xê nê ca",
    "Stoic": "Xtô ích",
    "Heian": "Hây an",
    "Sei Shonagon": "Xây Xô na gôn",
    "Freud": "Phờ rớt",
    "Shakespeare": "Sếch xpia",
    "Rosalind": "Rô xa lin",
    "Josephine Baker": "Giô xê phin Bây cơ",
    "Marlene Dietrich": "Mác len Đi trích",
    "Montgomery Clift": "Mông gô mê ri Cờ líp",
    "John Wayne": "Giôn Oai nơ",
    "John Kenedy": "Giôn Ke ne đi",
    "Victoria": "Víc to ri a",
    "Satan": "Sa tăng",
    "Big Ben": "Bích Ben",
    "Wales": "Oen",
    "Kenneth Anger": "Ken nét An gơ",
    "Mick Jagger": "Mích Gác gơ"
}

numbers = {
    "1913": "một ngàn chín trăm mười ba",
    "1915": "một ngàn chín trăm mười lăm",
    "1919": "một ngàn chín trăm mười chín",
    "1920": "một ngàn chín trăm hai mươi",
    "1926": "một ngàn chín trăm hai mươi sáu",
    "1870": "một ngàn tám trăm bảy mươi",
    "1878": "một ngàn tám trăm bảy mươi tám",
    "1882": "một ngàn tám trăm tám mươi hai",
    "1897": "một ngàn tám trăm chín mươi bảy",
    "1830": "một ngàn tám trăm ba mươi",
    "1840": "một ngàn tám trăm bốn mươi",
    "18": "mười tám",
    "17": "mười bảy",
    "42": "bốn mươi hai",
    "31": "ba mươi mốt",
...
for num, viet in numbers.items():
    text = text.replace(num, viet)
    
text = text.replace("thế kỷ 17", "thế kỷ mười bảy")
text = text.replace("thế kỷ 18", "thế kỷ mười tám")
text = text.replace("thế kỷ 19", "thế kỷ mười chín")
text = text.replace("Bốn kỵ sĩ", "bốn kỵ sĩ")
text = text.replace("hai cuộc hôn nhân", "hai cuộc hôn nhân") # etc

# Remove quotes, dashes, colons, parentheses
text = re.sub(r'["“”]', '', text)
text = re.sub(r'[-–—]', ',', text)
text = re.sub(r'[:()]', ',', text)

# End sentences with dot, replace ..., and end paragraphs with . ......
...
for p in paragraphs:
    p = p.replace('...', ',')
    p = re.sub(r',+', ',', p)
    # Ensure it ends with a dot. Wait, paragraphs will end with `. ......`
    if not p.endswith('.'):
        p += '.'
    # Then append ` ......`
    p += ' ......'
...
for p in new_paragraphs:
    # We will split paragraphs into sentences for chunking, but we want to keep the paragraph marker.
    # Actually it's better to chunk by sentences. A sentence ends with `.`, `?`, `!`.
    # Wait, the prompt says split 15-20 sentences per file.
    
    # We can use regex to split sentences, keeping the punctuation.
    splits = re.split(r'(?<=[.?!])\s+', p)
    for s in splits:
        if s.strip():
            sentences.append(s.strip())

# Some sentences will be just `......`, we should merge them back to the previous sentence.
merged_sentences = []
for s in sentences:
    if s == '......':
...
for s in merged_sentences:
    ...
    current_len += 1
    if current_len >= 17: # 17 is between 15 and 20
...
os.makedirs('/mnt/d/Solution/Audio-Book-Refactor/Kich-ban-clipchamp/Nghe-thuat-quyen-ru/Chuong-5', exist_ok=True)
for i, chunk in enumerate(chunks):
    content = "\\n\\n".join(chunk) # Re-add newlines for TTS clarity, though prompt says "Xuống dòng và dùng `. ......`"
    # Actually wait, Clipchamp doesn't care much about newlines in the middle of a sentence, but let's just make it a readable block.
    # The prompt says: "Nghỉ chuyển đoạn/chuyển ý: Xuống dòng và dùng `. ......` để tạo khoảng lặng để người nghe suy ngẫm."
    # Let's join sentences with space, and paragraphs already have ` ......` which handles the paragraph end.
    
    # Let's fix the chunking to preserve paragraph breaks properly.
    text_content = " ".join(chunk)
    text_content = text_content.replace(' ......', '. ......\\n')
    text_content = text_content.replace('. . ......', '. ......')
    
    with open(f'/mnt/d/Solution/Audio-Book-Refactor/Kich-ban-clipchamp/Nghe-thuat-quyen-ru/Chuong-5/Kich-ban-{i+1}.txt', 'w', encoding='utf-8') as f:
        f.write(text_content)

print(f"Generated {len(chunks)} files.")
