...
title Universal Audiobook Pipeline - Cài Đặt Tự Động 1-Click

echo ================================================================================
echo    HỆ THỐNG SẢN XUẤT SÁCH NÓI VẠN NĂNG - UNIVERSAL AUDIOBOOK PIPELINE v2.0
echo    CÀI ĐẶT TỰ ĐỘNG 1-CLICK (ONE-CLICK SETUP WIZARD)
echo ================================================================================
echo.
...
echo     -> Đang cài đặt thư viện từ requirements.txt (edge-tts, mutagen, pypdf, imageio-ffmpeg)...
...
if %ERRORLEVEL% equ 0 (
    ...
    if exist "tools\ffmpeg\ffmpeg.exe" (
        ...
        if exist "ffmpeg-7.0.2-amd64-static\ffmpeg.exe" (
            echo [✓] Đã phát hiện FFmpeg tại: ffmpeg-7.0.2-amd64-static\ffmpeg.exe
        ) else (
...
echo    1. Nhấp đúp chuột vào file: run_audiobook.bat (để mở menu chọn nhanh)
echo    2. Hoặc ra lệnh trực tiếp cho AI trong Antigravity IDE / Claude / ChatGPT
echo.
pause
