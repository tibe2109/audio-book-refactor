...
1. **Trường Hợp 1: Ngắt Nghỉ Điểm Nhấn Thính Giác (Contextual Anchoring & Emphasis Pause):**
   ...
   - **Kỹ thuật phát thanh:** Bắt buộc có dấu phẩy `, ` để tạo một nhịp dừng vi mô (micro-pause 200ms - 300ms). Nhịp nghỉ này giúp tai người nghe kịp tiếp nhận ý niệm bằng tiếng Việt trước, sau đó đón nhận phát âm chuẩn quốc tế từ giọng Brian để ghi nhớ sâu sắc.
   - **Ví dụ chuẩn:**
   ...
   - **Cơ chế âm thanh:** Giữ nguyên dấu phẩy `, `, đệm khẩu hình phát thanh (`apad=pad_dur=0.04`), tạo nhịp thở tự nhiên.

2. **Trường Hợp 2: Đọc Liền Mạch Ngữ Pháp Tự Nhiên (Seamless Grammatical Continuity):**
   ...
   - **Kỹ thuật phát thanh:** Đọc và ghép âm **liền mạch 100%, không được ngắt quãng, khựng tiếng hay tạo cảm giác giật cục**.
   - **Ví dụ chuẩn:**
     - `"các Project Manager thường tuân thủ một quy trình chặt chẽ"` ("các" nối liền mạch sang "Project Manager")
     ...
     - `"Một quán bar nhỏ"` ("Một quán" nối liền mạch sang "bar")
   - **Cơ chế âm thanh (Zero-Gap Splicing):**
     ...
     - Áp dụng bộ lọc an toàn `areverse,silenceremove=start_periods=1:start_duration=0.05:start_threshold=-45dB,areverse` để gọt sạch khoảng lặng chết do API TTS sinh ra ở đuôi phân đoạn mà không bao giờ cắt mất nội dung phát âm bên trong. Tuyệt đối không dùng `stop_periods=1` vì sẽ gây cụt tiếng (truncate) tại khoảng dừng micro-pause đầu tiên.
