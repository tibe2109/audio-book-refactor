...
def main():
    ...
    # Step 1: Clean all old final files
    print("=== STEP 1: PURGING ALL OLD FINAL AUDIO FILES ===")
    old_finals = [f for f in os.listdir(final_dir) if f.startswith('Final_Audio_') and f.endswith('.mp3')]
    for f in old_finals:
        os.remove(os.path.join(final_dir, f))
    print(f"Purged {len(old_finals)} old final files.")
    
    # Step 2: Build dual BGM schedule
    ...
    for f in full_files:
        ...
        final_mp3 = os.path.join(final_dir, f.replace('Full_', 'Final_Audio_'))
    ...
    print(f"\n=== STEP 2: MASTERING {len(schedule)} CHAPTERS WITH DUAL BGM COORDINATION ===")
