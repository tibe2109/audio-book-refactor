...
# -*- coding: utf-8 -*-
"""
Builder for Chapter 03: Courage fulfills us
"""
import os
...
C03_SUMMARY = """# Tóm Tắt: CHƯƠNG 3: LÒNG DŨNG CẢM MANG LẠI SỰ TRỌN VẸN VÀ Ý NGHĨA
...
```mermaid
mindmap
  root((DŨNG CẢM & Ý NGHĨA TRỌN VẸN))
    🏛️ Cuộc Đời Trọn Vẹn
...
C03_MINDMAP = """# 🧠 BẢN ĐỒ TƯ DUY TINH GỌN (4-5 TẦNG): CHƯƠNG 3 - LÒNG DŨNG CẢM MANG LẠI SỰ TRỌN VẸN VÀ Ý NGHĨA

## 1. Sơ Đồ Tư Duy Trực Quan (Mermaid Mindmap Tinh Gọn)

```mermaid
mindmap
  root((Ý NGHĨA TRỌN VẸN))
    🏛️ Cuộc Đời Tốt Đẹp
      Aristotle
        Eudaimonia
          Tự quyết cá nhân
...
c3_dir = os.path.join(BASE_DIR, "03-Courage-fulfills-us")
with open(os.path.join(c3_dir, "summary.md"), "w", encoding="utf-8") as f:
    f.write(C03_SUMMARY.strip() + "\n")
with open(os.path.join(c3_dir, "mindmap.md"), "w", encoding="utf-8") as f:
    f.write(C03_MINDMAP.strip() + "\n")
print("[✓] Chapter 03 files written successfully.")
