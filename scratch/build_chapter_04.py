...
# -*- coding: utf-8 -*-
"""
Builder for Chapter 04: Four fears hold courage back
"""
import os
...
C04_SUMMARY = """# Tóm Tắt: CHƯƠNG 4: BỐN NỖI SỢ CẢN TRỞ LÒNG DŨNG CẢM
    ...
    🗣️ Hai Mặt Trận Đối Thoại
      ...
      Đối thoại ngang hàng
        Không dựa quyền thế
          Góp ý bạn thân
            Tránh đùn đẩy việc
    🛡️ Bốn Nỗi Sợ Tiến Hóa
      Rủi ro kinh tế
        Mất việc mất thưởng
          Chiếc khiên tài chính
      Rủi ro xã hội
        Bị tẩy chay bầy đàn
          Sợ bị cô lập ăn trưa
      Rủi ro tâm lý
        Sợ cảm giác kém cỏi
          Hội chứng Imposter
      Rủi ro thể chất
...
C04_MINDMAP = """# 🧠 BẢN ĐỒ TƯ DUY TINH GỌN (4-5 TẦNG): CHƯƠNG 4 - BỐN NỖI SỢ CẢN TRỞ LÒNG DŨNG CẢM

## 1. Sơ Đồ Tư Duy Trực Quan (Mermaid Mindmap Tinh Gọn)

```mermaid
mindmap
  root((BỐN NỖI SỢ CÔNG SỞ))
    📋 Phổ 35 Hành Vi
      Nghiên cứu Detert
        75% đồng thuận
          Bình thường lý thuyết
            Hiếm hoi thực tế
      Đổi mới sáng tạo
        Thử nghiệm mới
          Chấp nhận rủi ro
...
c4_dir = os.path.join(BASE_DIR, "04-Four-fears-hold-courage-back")
with open(os.path.join(c4_dir, "summary.md"), "w", encoding="utf-8") as f:
    f.write(C04_SUMMARY.strip() + "\n")
with open(os.path.join(c4_dir, "mindmap.md"), "w", encoding="utf-8") as f:
    f.write(C04_MINDMAP.strip() + "\n")
print("[✓] Chapter 04 files written successfully.")
