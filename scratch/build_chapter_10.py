...
# -*- coding: utf-8 -*-
"""
Builder for Chapter 10: Build your courage ladder
"""
import os
...
C10_SUMMARY = """# Tóm Tắt: CHƯƠNG 10: XÂY DỰNG NẤC THANG LÒNG DŨNG CẢM CỦA RIÊNG BẠN
    ...
    🪜 Thiết Kế Nấc Thang
      Ba tầng thử thách
        Đáy thang nhẹ nhàng
          Giữa thang vừa phải
            Đỉnh thang vĩ đại
      Bản năng thường nhật
        Leo từng bậc
          Nâng ngưỡng chịu đựng
            Căn cước dũng cảm
```
"""

C10_MINDMAP = """# 🧠 BẢN ĐỒ TƯ DUY TINH GỌN (4-5 TẦNG): CHƯƠNG 10 - XÂY DỰNG NẤC THANG LÒNG DŨNG CẢM CỦA RIÊNG BẠN

## 1. Sơ Đồ Tư Duy Trực Quan (Mermaid Mindmap Tinh Gọn)

```mermaid
mindmap
  root((NẤC THANG CAN ĐẢM))
    ⚠️ Bẫy Cực Đoan
      Extreme Goals
        Nghĩ điều kinh hoàng
...
c10_dir = os.path.join(BASE_DIR, "10-Build-your-courage-ladder")
with open(os.path.join(c10_dir, "summary.md"), "w", encoding="utf-8") as f:
    f.write(C10_SUMMARY.strip() + "\n")
with open(os.path.join(c10_dir, "mindmap.md"), "w", encoding="utf-8") as f:
    f.write(C10_MINDMAP.strip() + "\n")
print("[✓] Chapter 10 files written successfully.")
